...
//Protothread that handles Bluetooth data
static PT_THREAD (protothread_ble(struct pt *pt))
{
    PT_BEGIN(pt);

    static char video_buffer[32];
    static char characteristic_value_tx[32];  // Buffer for ADC values

    // Initialize MyoWare sensor ADC
    adc_select_input(0);  // Select GPIO26 (ADC1) for MyoWare sensor
    ...
    while(1) {
        ...
        // Update characteristic D with the ADC value
        set_characteristic_d_value(adc_value);
        
        // Yield to the Bluetooth stack and other tasks
        PT_YIELD_usec(500000);  // Adjust delay as needed (500ms)
    }
